...
//RANK_AVLTREE_TESTS:
    ...
    ServersTree* rt_1 = new ServersTree();
    ServersTree* rt_2 = new ServersTree();
    ServersTree* rt_3 = new ServersTree();
    for(int i=0; i<5; ++i)
        rt_1->AddToTree({i, i});
    for(int i=0; i<5; ++i)
        rt_2->AddToTree({i+10, i+10});
    List<SRKey>* list = UnionFind::merge(rt_1, rt_2);
    rt_3 = ServersTree::FromSortedList(*list);

// UF_TESTS:

    UnionFind* uf_1 = new UnionFind(10);
    for (int i=1; i<=10; ++i){
        assert(uf_1->Find(i) == i);
    }
    assert(uf_1->Union(1,2) == 1); // 1 size is 2
    assert(uf_1->Union(3,4) == 3); // 3 size is 2
    assert(uf_1->Union(2,4) == 1); // 1 size is 2
    assert(uf_1->Find(4) == 1); //check route has been successfully shrinked!!!
    assert(uf_1->Find(3) == 1); //check route has been successfully shrinked!!!
    assert(uf_1->Union(5,6) == 5); // 5 size is 2
    assert(uf_1->Union(7,8) ==7 ); // 7 size is 2
    assert(uf_1->Union(9,8) == 7); // 7 size is 3
    assert(uf_1->Union(5,9) == 7);
    assert(uf_1->Union(2,8) == 7);

    delete uf_1;

    UnionFind* uf_2 = new UnionFind(10);
    for (int i=1; i<=10; ++i){
        assert(uf_2->Find(i) == i);
    }
    assert(uf_2->Union(2,2)==2);
    assert(uf_2->Union(3,4)==3);
    assert(uf_2->Union(1,4)==3);
    assert(uf_2->Find(1) == 3); //check route has been successfully shrinked!!!
    assert(uf_2->Union(5,1)==3);
    assert(uf_2->Union(6,1)==3);
    assert(uf_2->Union(7,6)==3);
    assert(uf_2->Union(9,8)==9);
    assert(uf_2->Union(10,4)==3);
    assert(uf_2->Find(8) ==9);
    assert(uf_2->Find(5) ==3);
    assert(uf_2->Find(10)==3);
